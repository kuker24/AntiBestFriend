---
name: wizard
description: "Generate an interactive bash wizard for steps only a human can perform (dashboard, OAuth, secret provision, cutover). Do not use for work the agent can do itself."
...
**Secrets.** Never print a captured secret, token, or password in the session, in the wizard summary, or in logs. `ask_secret` hides input. `write_env` / `set_secret` may name the *key*, never the value. Do not `echo` secret variables. Do not generate `curl | sh` or `wget | sh`.
