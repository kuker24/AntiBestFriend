...
Don't invent jargon. Use the words a developer would say out loud: "move", "delete", "a budget that only decreases", not "evacuate", "ratchet", or "endgame". A named pattern is fine when the doc says what it means the first time. Precision beats style. Read `~/.claude/grokbestfriend-claude/rules/03-prose-discipline.md` when cutting filler. Do not invoke `/unslop` from this skill.
...
- Apply the prose-discipline rules in `~/.claude/grokbestfriend-claude/rules/03-prose-discipline.md`. Do not auto-run `/unslop`. `/unslop` is a separate manual skill for user-facing voice, not documentation structure.
